...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await generateText({ model: boardModel(), system: persona.instructions, prompt: memberPrompt(input, "Form your private provisional position on the decision. Each field is one or two sentences in your own voice, specific to this briefing: recommendation, the central reasoning, your most important concern, and one question or assumption worth testing."), output: Output.object({ schema: openingPositionSchema }), maxOutputTokens: 320, providerOptions: lowReasoning, abortSignal: request.signal });

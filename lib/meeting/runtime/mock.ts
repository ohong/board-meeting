...
export function createMockRuntime(options: MockRuntimeOptions = {}): BoardRuntime & { pacingScale: number; reactMany(inputs: ReactInput[], signal: AbortSignal): Promise<ReactResult[]> } {
  ...
  const runtime = {
    ...
    async reactMany(inputs: ReactInput[], signal: AbortSignal): Promise<ReactResult[]> { await wait(180 * scale, signal); maybeFail("react"); return inputs.map((i) => i.slug === "daniel-ek" && i.lastSpeakerId === "david-heinemeier-hansson" ? { reaction: "disagree", urgency: 10, wantsToRebut: true } : { reaction: i.slug === "lulu-cheng-meservey" ? "concern" : null, urgency: i.ownStatements.length ? 3 : 7, wantsToRebut: false }); },
    async closingComment(input, signal) { await wait(250 * scale, signal); maybeFail("closingComment", input.slug); if (input.slug === "daniel-ek") return "Measure the door before you brick it up."; if (input.slug === "david-heinemeier-hansson") return "Stop paying support costs for users who will never pay you."; if (input.slug === "lulu-cheng-meservey") return "Whatever you decide, say it first and say it plainly."; return `${input.participants.find((p) => p.id === input.slug)?.name ?? "I"}: test the key assumption before committing.`; },
...
function scriptedTurn(input: TurnInput, count: number): string {
  ...
  return `${input.participants.find((p) => p.id === input.slug)?.name ?? "I"}: narrow the experiment, name the tradeoff, and define the evidence that would change the board's view.`;
}

...
export const SPEECH_CARD_W = 248;

/** Hard cap on its height, enforced by clamping the text to four lines. */
export const SPEECH_CARD_MAX_H = 132;

/**
 * The card lies on the table, leaning a little toward whoever is speaking.
 *
 * The collision-free region is smaller than the table looks and is not centred:
 * every seat reserves a 150px-wide label column whose ink runs ~100px below its
 * portrait centre, and the chair's portrait reaches up to y = 102. These values
 * (248 x 132 at y = 20, leaning at most 24px) were verified against the ink of
 * every label on three-, four-, five- and six-member boards, with the speaker at
 * both extremes of the arc. Changing the card's size, the seat labels, or
 * SPAN_BY_COUNT can reintroduce an overlap, so re-check it if you do.
 *
 * The card is capped rather than free-flowing on purpose: the full turn is
 * already streaming into the minutes immediately to its right.
 */
const LEAN_X = 24;
const CARD_Y = 20;
...
export function speechCardPoint(angleDeg: number): ScenePoint {
  ...
  return { x: Math.max(-LEAN_X, Math.min(LEAN_X, seat.x * 0.46)), y: CARD_Y };
}

/**
 * Centre of the pool of light on the table in front of the speaker. Closer to
 * the table centre than the seat itself, the way a lamp over a table lights the
 * surface someone is leaning over rather than the person.
 */
export function spotlightPoint(angleDeg: number): ScenePoint {
  return pointAt(angleDeg, RING_RX * 0.5, RING_RY * 0.46);
}

/** Size of that pool of light. */
export const SPOTLIGHT_W = 400;
export const SPOTLIGHT_H = 280;
